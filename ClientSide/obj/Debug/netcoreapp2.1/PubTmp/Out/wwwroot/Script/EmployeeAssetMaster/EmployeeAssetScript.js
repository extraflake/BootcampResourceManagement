...
function Save() {
    ...
    $.ajax({
        ...
        if (result.StatusCode == 200) {
            ...
            window.location.href = "/HistoryAssets/";
        }
        else {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (index, val) {
                debugger;
                receive_date = moment(val.ReceiveDate).format('MM/DD/YYYY');
                if (val.ReturnDate != null) {
                    return_date = moment(val.ReturnDate).format('MM/DD/YYYY');
                }
                else {
                    return_date = "-";
                }
                ...
                html += '<td style="text-align:center">' + receive_date + '</td>';
                html += '<td style="text-align:center">' + return_date + '</td>';
                html += '</tr>';
                i++;
            });
            $('.tbody').html(html);
